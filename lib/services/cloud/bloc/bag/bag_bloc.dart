...
class BagBloc extends Bloc<BagEvent, BagState> {
  final BagRepository _bagRepository = BagRepository();
        ...
        try {
          if (state is BagLoadedState) {
            ...
            if (bag.BagProducts == null) {
              final bagProduct = await _bagRepository.createBagProduct(bag, event.product, event.size);
    ...
    on<BagItemCountIncreaseEvent>(
      (event, emit) {
        try {
          if (state is BagLoadedState) {
            Bag bag = (state as BagLoadedState).bag;
            bag.BagProducts![event.index] = bag.BagProducts![event.index].copyWith(quantity: event.quantity);
            // update bagproduct quantity
    ...
    on<BagItemSizeChangeEvent>(
      (event, emit) {
        try {
          if (state is BagLoadedState) {
            Bag bag = (state as BagLoadedState).bag;
            bag.BagProducts![event.index] = bag.BagProducts![event.index].copyWith(size: event.size);
            // update bagproduct quantity
            _bagRepository.updateBagProduct(bag.BagProducts![event.index]);
            emit(BagLoadedState(bag: bag));
          }
      ...
      (event, emit) async {
        try {
          if (state is BagListOrdersLoadedState) {
            ...
            final orderIndex = orders.indexWhere((element) => element.id == event.bagProduct.bagID);
            Bag bag = orders[orderIndex];
            final bagProductIndex = bag.BagProducts!.indexWhere((element) => element.id == event.bagProduct.id);
            bag.BagProducts![bagProductIndex] = bag.BagProducts![bagProductIndex].copyWith(isRated: true);
            orders[orderIndex] = bag;

            _bagRepository.updateBagProduct(bag.BagProducts![bagProductIndex]);
            await _bagRepository.addReview(
                event.bagProduct.bagID, event.bagProduct.productID, event.rating, event.content);
            emit(BagListOrdersLoadedState(orders: orders));
          }
        } on Exception catch (e) {
          emit(BagErrorState(exception: e));
