...
enum CustomStepState {
  /// A step that displays its dotted in its circle.
  dotted,
...
/// CUSTOM TO ADJUST SIZE OF STEPPER
const double _kMarginPadding = _kStepSize;
...
class CustomStep {
  ...
  const CustomStep({
    ...
    this.state = CustomStepState.dotted,
    this.isActive = false,
    this.label,
  });
...
class _CustomStepperState extends State<CustomStepper> with TickerProviderStateMixin {
  ...
  Widget _buildIcon(int index) {
    if (widget.steps[index].state != _oldStates[index]) {
      return AnimatedCrossFade(
        ...
        crossFadeState:
            widget.steps[index].state == CustomStepState.error ? CrossFadeState.showSecond : CrossFadeState.showFirst,
        duration: kThemeAnimationDuration,
      );
    } else {
  ...
  Widget _buildVerticalHeader(int index) {
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: _kMarginPadding),
      child: Row(
        children: <Widget>[
          Column(
            children: <Widget>[
    ...
    return Stack(
      children: <Widget>[
        PositionedDirectional(
          start: _kMarginPadding,
          top: 0.0,
          bottom: 0.0,
          child: SizedBox(
            width: _kMarginPadding,
            child: Center(
              child: SizedBox(
    ...
    return Column(
      children: <Widget>[
        ...
        Expanded(
          child: ListView(
            physics: widget.physics,
            padding: const EdgeInsets.all(_kMarginPadding),
            children: <Widget>[
              AnimatedSize(
                curve: Curves.fastOutSlowIn,
                duration: kThemeAnimationDuration,
